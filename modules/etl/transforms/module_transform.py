...
class ModuleTransform (object):
  ...
  def basicTransform (self: object, df: pd.DataFrame) -> object:
    ...
    if self.vtype.isvalid(df, self.accessors):
      ...
      df = df[list(set(accessor["field"] for key, accessor in self.accessors.items()))]
      groups, value, func = self.transform["groups"], self.transform["value"], self.transform["func"]
      transformed = df.groupby(groups)[value][func]()
      # print(transformed)
      for record in transformed.to_dict("records"):
        record = {key: record[accessor["field"]] for key, accessor in self.accessors.items()}
