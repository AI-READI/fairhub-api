    ...
    def __init__(
        ...
        for i, transform in enumerate(self.transformList):
            if "name" not in transform:
                self.logger.error(
                    f"Transform at index {i} in transforms list missing name property"
                )
                self.valid_transforms = False
            if "method" not in transform:
                self.logger.error(
                    f"Transform at index {i} in transforms list missing method property"
                )
                self.valid_transforms = False
            if "accessors" not in transform:
                self.logger.error(
                    f"Transform at index {i} in transforms list missing accessors property"
                )
                self.valid_transforms = False
        if self.strict and not self.valid_transforms:
            raise ValueError(
                f"Missing properties in transforms argument, see log at {self.logging_config['filename']} for details"
            )
    ...
    def simpleTransform(self: object, df: pd.DataFrame) -> object:
        ...
        if self.vtype.isvalid(df, accessors):
            ...
            for record in transformed.to_dict("records"):
                record = {
                    key: self._setValueType(record, key, accessor)
