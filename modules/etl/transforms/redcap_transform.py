...
class REDCapTransform (object):
    ...
    def __init__ (self, config: dict) -> None:
        ...
        self._field_rgx["text"]        = re.compile(r"^[a-zA-Z0-9\-\_\(\)\[\]\&\+\?\!\$\*]{1,128}")
        self._field_rgx["descriptive"] = re.compile(r"^[a-zA-Z0-9\-\_\(\)\[\]\&\+\?\!\$\*]{1,128}")
        self._field_rgx["notes"]       = re.compile(r"^[a-zA-Z0-9\-\_\(\)\[\]\&\+\?\!\$\*]{1,128}")
        ...
        for transform, transform_kwdargs in self.post_merge_transforms:
            self.merged = self.apply_transform(self.merged, transform, transform_kwdargs)
...
#
    ...
    def get_report_pycap (self: object, report_name: str) -> Union[List[Dict[str, Any]], str, pd.DataFrame]:
        """
    ...
    def get_report_transforms (self: object, report_name: str) -> List[Tuple[str, Dict[str, Any]]]:
        """
...
#
    ...
    def _apply_report_transforms (self: object, report_name: str) -> None:
        ...
        for transform in report["transforms"]:
            ...
            report["transformed"] = self.apply_transform(report["transformed"], transform_name, transform_kwdargs)

        return self

    def apply_transform (self: object, df: pd.DataFrame, transform_name: str, transform_kwdargs: Dict[str, Any] = {}) -> pd.DataFrame:
...
#
    ...
    def _drop_columns (self: object, df: pd.DataFrame, columns: List[str] = [], annotation: List[Dict[str, Any]] = []) -> pd.DataFrame:
        columns = self._resolve_columns_with_dataframe(df = df, columns = columns)
        df = df.drop(columns = columns)
        return df

    @classmethod
    def drop_columns (self: object, df: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
        """
    ...
    def _keep_columns (self: object, df: pd.DataFrame, columns: List[str] = [], annotation: List[Dict[str, Any]] = []) -> pd.DataFrame:
        columns = set(df.columns) - set(self._resolve_columns_with_dataframe(df = df, columns = columns))
        df = df.drop(columns = columns)
        return df

    @classmethod
    def keep_columns (self: object, df: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
        """
    ...
    def _append_column_suffix (self: object, df: pd.DataFrame, columns: List[str] = [], suffix: str = "", separator: str = "", annotation: List[Dict[str, Any]] = []) -> pd.DataFrame:
        columns = self._resolve_columns_with_dataframe(df = df, columns = columns)
        df[columns] = df[columns].rename(mapper = lambda name: f"{namer}{separator}{suffix}")
        return df

    @classmethod
    def append_column_suffix (self: object, df: pd.DataFrame, columns: List[str] = [], suffix: str = "", separator: str = "") -> pd.DataFrame:
        """
        ...
        snakecase.
        """
        return self._prepend_column_suffix(df = df, columns = transform_columns, suffix = suffix, separator = separator)

    #
    # Transform - Prepend Column Prefix
    #

    def _prepend_column_prefix (self: object, df: pd.DataFrame, columns: List[str] = [], prefix: str = "", separator: str = "", annotation: List[Dict[str, Any]] = []) -> pd.DataFrame:
        columns = self._resolve_columns_with_dataframe(df = df, columns = columns)
        df[columns] = df[columns].rename(mapper = lambda name: f"{prefix}{separator}{name}")
        return df

    @classmethod
    def prepend_column_prefix (self: object, df: pd.DataFrame, columns: List[str] = [], prefix: str = "", separator: str = "") -> pd.DataFrame:
        """
        ...
        snakecase.
        """
        return self._prepend_column_prefix(df = df, columns = columns, prefix = prefix, separator = separator)

    #
    # Transforms - Remap Values by Columns
    #

    def _remap_values_by_columns (self: object, df: pd.DataFrame, columns: List[str], value_map: Dict[str, Any] = {}, annotation: List[Dict[str, Any]] = []) -> pd.DataFrame:

        # Resolve Mappable Fields and Available Value Maps
        columns = self._resolve_columns_with_dataframe(df = df, columns = columns)

        if len(value_map) > 0:
            mappable_fields = [{"name": column, "options": value_map} for column in columns]
        else:
            mappable_fields = [field for field in annotation if field["options"] is not None and field["name"] in columns]

        for mappable_field in mappable_fields:
            column, value_map = mappable_field["name"], mappable_field["options"]
            for i, value in enumerate(df[column]):
                subvalues = [subvalue.strip() for subvalue in str(value).split(",") if len(subvalue) > 0]
                df[column][i] = self.multivalue_separator.join(
                    [value_map[subvalue] for subvalue in subvalues if subvalue in value_map.keys()]
                )

        return df

    @classmethod
    def remap_values_by_columns (self: object, df: pd.DataFrame, columns: List[str],  value_map: Dict[str, Any] = {}) -> pd.DataFrame:
        """
        ...
        """
        return self._remap_values_by_columns(df = df, columns = columns, value_map = value_map)
...
#
    ...
    def _drop_rows (self: object, df: pd.DataFrame, columns: List[str] = [], condition: Callable = lambda column: column == "", annotation: List[Dict[str, Any]] = []) -> pd.DataFrame:
        columns = self._resolve_columns_with_dataframe(df = df, columns = columns)
        df = df[~df[columns].apply(lambda column: column.apply(condition)).any(axis = 1)]
        return df

    @classmethod
    def drop_rows (self: object, df: pd.DataFrame, columns: List[str], condition: Callable = lambda column: column == "") -> pd.DataFrame:
        """
...
#
    ...
    def aggregate_repeat_instrument_by_index (self: object, df: pd.DataFrame, aggregator: str = "max", dtype: Callable = float) -> pd.DataFrame:
        ...
        field.
        """
        return self._aggregate_repreat_instrument_by_index(df = df, aggregator = aggregator, dtype = dtype)
...
#
    ...
    def _merge_reports (self: object, receiving_report_name: str, merge_steps: List[Tuple[str, Dict[str, Any]]]) -> pd.DataFrame:
        """
        ...
        if len(merge_steps) > 0:
            for providing_report_name, merge_kwdargs in merge_steps:
                ...
                df_receiving_report = df_receiving_report.merge(df_providing_report, **merge_kwdargs)
        else:
            self.logger.warn(f"Unable to Merge – No merge steps provided, returning receiving_report pd.DataFrame.")
...
#
    ...
    def _resolve_columns_with_dataframe (self: object, df: pd.DataFrame, columns: List[str]) -> List[str]:
        """
        ...
        if len(requested_columns) == 0:
            self.logger.warn(f"Unexpected Transform – columns parameter has no values. Defaulting to df.columns")
            resolved_columns = [*available_columns]
        elif len(available_columns & requested_columns) == 0:
            self.logger.warn(f"Unexpected Transform – none of the values in the columns parameter were found in df.columns. Defaulting to df.columns")
            resolved_columns = [*available_columns]
        elif len(requested_columns - available_columns) > 0:
            self.logger.warn(f"Unexpected Transform – df.columns missing values present in columns parameter: {', '.join([*requested_columns - available_columns])}. Continuing with union.")
    ...
    def _get_redcap_type_metadata (self: object, df: pd.DataFrame) -> List[Dict[str, Any]]:
        """
            ...
            if field["field_name"] in columns:
                ...
                if field_type in complex_types:
                    ...
                    for option in field["select_choices_or_calculations"].split("|"):
                        k, v = option.split(",")[0], (",".join(option.split(",")[1:])).strip()
    ...
    def export_raw (self: object, path: str = "", separator: str = "\t", filetype: str = ".tsv") -> object:
        for report_name, report_object in self.reports.items():
            filename = f"{report_name}_raw{filetype}"
            filepath = os.path.join(path, filename)
            transformed = report_object["df"]
            transformed.to_csv(filepath, sep = separator, quoting = csv.QUOTE_NONNUMERIC, float_format = self.csv_float_format)
        return self

    # Export Transformed Reports
    def export_transformed (self: object, path: str = "", separator: str = "\t", filetype: str = ".tsv") -> object:
        for report_name, report_object in self.reports.items():
    ...
    def export_merged_transformed (self: object, path: str = "", separator: str = "\t", filetype: str = ".tsv") -> object:
        filename = f"transformed-merged_redcap-extract{filetype}"
        filepath = os.path.join(path, filename)
        self.merged.to_csv(filepath, sep = separator, quoting = csv.QUOTE_NONNUMERIC, float_format = self.csv_float_format)
        return self
