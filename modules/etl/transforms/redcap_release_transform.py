...
import re, os, logging, copy, json, io
...
class RedcapReleaseTransform(object):
    ...
    def run (self):
        ...
        self.metadata = self.get_stored_project_metadata(
            ...
            f"{self.redcap_metadata_config['filepath']}/{self.redcap_metadata_config['filename']}"
        )
    ...
    def get_stored_project_metadata(self, connection_string: str, container_name: str, blob_path: str) -> dict:

        # Connect to Azure Blog Storage
    ...
    def get_stored_report(self, connection_string: str, container_name: str, blob_path: str) -> pl.DataFrame:

        self.logger.info(f"Downloading report blob: {blob_path}")
        try:
            blob_service_client = BlobServiceClient.from_connection_string(connection_string)
            container_client = blob_service_client.get_container_client(container_name)
            blob_client = container_client.get_blob_client(blob_path)

            # Get Blob as bytes stream
            download_stream = blob_client.download_blob()
            blob_bytes = download_stream.readall()

            # Use io.BytesIO to wrap the bytes content into a file-like object
            # Polars can read directly from this buffer.
            report_buffer = io.BytesIO(blob_bytes)

            # Read CSV directly into a Polars DataFrame
            ...
            self.logger.error(f"Failed to retrieve or read Azure Blob report at {blob_path}: {e}")
            # Return an empty Polars DataFrame on failure
            return pl.DataFrame({})
    ...
    def _merge_reports(
        ...
        if len(merge_steps) > 0:
            for merge_step in merge_steps:
                ...
                # Ensure Reports Aren't None
                if df_receiving_report is None or df_providing_report is None:
                    self.logger.warn(f"Skipping merge step: {merge_step}")
                    continue
    ...
    def apply_transform(
        ...
        df = getattr(self, f"_{transform_name}")(df, **transform_kwdargs)
        return df
