...
class StudyDesign(db.Model):  # type: ignore
    ...
    def updating_from_integration(self, data: dict):
        ...
        self.design_observational_model_list = data.get("designModule", {}).get("observationalModel", "")
        self.design_time_perspective_list = data.get("designModule", {}).get("timePerspective", "")
        self.phase_list = data.get("designModule", {}).get("phases", "")
        self.design_allocation = data.get("designModule", {}).get("designInfo", {}).get("allocation", "").capitalize()
        self.design_primary_purpose = data.get("designModule", {}).get("designInfo", {}).get("primaryPurpose", "").capitalize()
        self.design_intervention_model = data.get("designModule", {}).get("designInfo", {}).get("interventionModel", "").capitalize()
        val = data.get("designModule", {}).get("designInfo", {}).get("maskingInfo", {}).get("masking", "")
        self.design_masking = "None (Open Label)" if val == "NONE" else val

        self.enrollment_count = data.get("designModule", {}).get("enrollmentInfo", {}).get("count", "")
        self.enrollment_type = data.get("designModule", {}).get("enrollmentInfo", {}).get("type", "")
